    ...
    <section className="py-16 md:py-24 px-4">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-12 md:mb-20">
          ...
          <p className="text-lg md:text-2xl text-gray-200 px-4 drop-shadow-md">
            Everything you need to know about Marketplace Credits
          </p>
        </div>
        
        <div className="bg-gradient-to-br from-dcl-ruby/20 to-dcl-orange/20 backdrop-blur-md rounded-2xl md:rounded-3xl border border-dcl-ruby/30 p-6 md:p-10 shadow-lg">
          <Accordion type="single" collapsible className="space-y-4 md:space-y-6">
            {faqs.map((faq, index) => (
              <AccordionItem 
                key={index} 
                value={`item-${index}`}
                className="border border-dcl-ruby/20 rounded-xl md:rounded-2xl px-4 md:px-8 bg-gradient-to-r from-dcl-ruby/10 to-dcl-orange/10 hover:from-dcl-ruby/20 hover:to-dcl-orange/20 transition-all duration-300 backdrop-blur-sm shadow-md"
              >
                <AccordionTrigger className="text-dcl-white hover:text-dcl-ruby text-left font-bold py-6 md:py-8 text-base md:text-lg drop-shadow-sm">
                  {faq.question}
                </AccordionTrigger>
                <AccordionContent className="text-gray-200 pb-6 md:pb-8 leading-relaxed text-base md:text-lg drop-shadow-sm">
                  {faq.answer}
                </AccordionContent>
              </AccordionItem>
            ))}
